    ...
    for picture in pictures:
        ...
        try:
            ...
            new_thumbpath = os.path.join(
                conf.SHELTERS_PICTURES_SERVER_PATH, str(picture.shelter_id), thumbname
            )
            if os.path.exists(new_thumbpath):
                if (
                    db.session.query(ShelterPicture)
                    .filter_by(file_name=thumbname)
                    .first()
                ):
                    continue
            ...
            print(
                "Failed to create thumbnail for shelter {}, file {}".format(
                    picture.shelter_id, picture.file_name
                )
            )
            continue
