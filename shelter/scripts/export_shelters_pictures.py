...
def export_shelters_pictures(dump_dir, foldername):
    ...
    if os.path.exists(os.path.join(dump_dir, foldername)):
        print(
            'Export aborted: Directory "{}" already exists. Erase it or choose a new directory'.format(
                foldername
            )
        )
        sys.exit(1)
    ...
    query = (
        db.session.query(Property.shelter_id.label("db_id"), Value.name.label("id"))
    ...
    for root, dirs, files in os.walk(picpath):
        ...
        if len(files) != 0:
            for f in files:
                ...
                shutil.copy(
                    os.path.join(root, f),
                    os.path.join(dst_published, id[int(os.path.basename(root))]),
                )
