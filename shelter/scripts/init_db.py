        ...
        for line, row in enumerate(structure):
            ...
            if cardinality in ("single choice", "multiple choice"):
                ...
                for value in row[6].split(";"):
                    ...
                    if not value:
                        value = models.Value(name=value_name, attribute_id=attribute.id)
            ...
            attribute.is_editable = (
                "automatic" not in row[8] and "not_editable" not in row[8]
            )
                    ...
                    for drawning in drawnings:
                        if number in drawning:
                            new_picture = models.AttributePicture(
                                file_name=os.path.basename(drawning), language_code="en"
                            )
                            attribute.pictures.append(new_picture)
                            db.session.add(new_picture)

                            path = os.path.join(
                                conf.ATTRIBUTES_PICTURES_SERVER_PATH, "en"
                            )
