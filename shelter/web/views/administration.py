...
def dashboard():
        ...
        if "shelters" in request.form:
            ...
            if file and allowed_file(file.filename, conf.ALLOWED_EXTENSIONS_CSV):
                ...
                launch_background_process(
                    ["import_shelters", current_user.name, filepath]
                )
            ...
            if file and allowed_file(file.filename, conf.ALLOWED_EXTENSIONS_ARCHIVE):
                ...
                launch_background_process(
                    ["import_shelters_pictures", "/tmp/sdPictures/"]
                )
    ...
    nb_unpublished_shelters = Shelter.query.filter(
        Shelter.is_published == False
    ).count()
...
def publish_shelter(shelter_id=None):
    ...
    if shelter:
        shelter.is_published = request.args.get("publish", not shelter.is_published)
...
def process_user_form(user_id=None):
    ...
    if user_id is not None:
        ...
        if not form.validate():
            return render_template(
                "/admin/create_user.html", form=form, message="Some errors were found"
            )
        user = User.query.filter(User.id == user_id).first()
...
def attributes(category_id=None, attribute_id=None):
    if attribute_id:
        ...
        return render_template("admin/values.html", attribute=attribute, form=form)

    if category_id:
        category = Category.query.filter(Category.id == category_id).first()
        form = CategoryForm(obj=category)
        return render_template("admin/attributes.html", category=category, form=form)
...
def attributes_add_pitures(category_id=None, attribute_id=None):
    ...
    if form.validate_on_submit():
        ...
        if form.name.data != attribute.name:
            ...
            Translation.query.filter(Translation.original == old_name).update(
                {Translation.original: form.name.data}
            )
            db.session.commit()
    ...
    if file and allowed_file(file.filename, conf.ALLOWED_EXTENSIONS_PICTURE):
        ...
        file.save(
            os.path.join(
                current_app.config["PUBLIC_PATH"] + "/pictures/attributes/en", filename
            )
        )

        new_picture = AttributePicture(
            file_name=filename, language_code="en", attribute_id=attribute_id
        )
