...
def details(shelter_id=0, section_name="", to_pdf=None):
    ...
    try:
        shelter, section, categories, pictures, documents = load_shelter_info(
            shelter_id, section_name
        )
...
def edit(shelter_id=0, section_name=""):
    ...
    if current_user.is_admin:
        ...
        flash("Only Admin or shelter creater have edit permission", "warning")
...
def delete(shelter_id=0, section_name=""):
    ...
    """
    shelter = Shelter.query.filter(
        Shelter.id == shelter_id, Shelter.user_id == current_user.id
    )
...
def get_media(shelter_id=0, section_name=""):
    ...
    if file and allowed_file(
        file.filename,
        conf.ALLOWED_EXTENSIONS_PICTURE.union(conf.ALLOWED_EXTENSIONS_DOCUMENT),
    ):
        ...
        if category_id:
            if "pictures" in request.form:
                new_media = ShelterPicture(
                    file_name=filename, shelter_id=shelter.id, category_id=category_id
                )
            if "documents" in request.form:
                new_media = ShelterDocument(
                    file_name=filename, shelter_id=shelter.id, category_id=category_id
                )
...
@shelter_bp.route(
    "/edit/multidocument/<int:shelter_id>/<category_id>/<section>", methods=["POST"]
)
@login_required
def get_multimedia_documents(shelter_id=0, category_id=2, section="Documents"):
    ...
    for f in request.files:
        ...
        if request.files[f] and not allowed_file(
            request.files[f].filename, conf.ALLOWED_EXTENSIONS_DOCUMENT
        ):
...
@shelter_bp.route(
    "/edit/multi/<int:shelter_id>/<category_id>/<section>", methods=["POST"]
)
# @login_required
def get_multi_media(shelter_id=0, category_id=2, section="Identification"):
    ...
    for f in request.files:
        ...
        if request.files[f] and allowed_file(
            ...
            exist_main = (
                db.session.query(ShelterPicture.is_main_picture)
                .filter(
                    ShelterPicture.is_main_picture == True,
            ...
            if im.size[0] > imgwidth:
                ...
                try:
                    ...
                    return (
                        "Failed to resize picture, please resize to "
                        + imgwidth
                        + " pixels width",
                        400,
                    )
            ...
            backup_dir = os.path.join(
                conf.SHELTERS_PICTURES_BACKUP_PATH, str(shelter_id_attribute)
            )
        ...
        if category_id:
            new_media = ShelterPicture(
                file_name=filename,
                is_main_picture=first,
                shelter_id=shelter.id,
                category_id=category_id,
            )

            new_thumbnail = ShelterPicture(
                file_name=thumbname,
                is_main_picture=first,
                shelter_id=shelter.id,
                category_id=category_id,
...
def delete_picture(picture_id=None):
    ...
    if picture:
        os.unlink(
            os.path.join(
                conf.SHELTERS_PICTURES_SERVER_PATH,
...
def delete_document(document_id=None):
    ...
    if document:
        os.unlink(
            os.path.join(
                conf.SHELTERS_DOCUMENTS_SERVER_PATH,
    ...
    return redirect(
        url_for(
            "shelter_bp.edit",
            shelter_id=document.shelter_id,
            section_name="general-Information",
        )
    )
