...
def jsonify_utf8(text):
    ...
    return Response(
        json.dumps(text, indent=3, sort_keys=False),
...
def populate_dictree(query, catquery, dictree, prettytext=False):
    ...
    for s in query:
        ...
        if prettytext:
            if s.type == "yes / no":
                dictree[s.shelter_id][s.supercategory_name]["Attributes"][
                    s.name
                ] = booleantext[int(s.value)]
            else:
                dictree[s.shelter_id][s.supercategory_name]["Attributes"][
                    s.name
                ] = s.value
        ...
        if p.shelter_id in dictree:
            if p.is_main_picture == True:
                if not dictree[p.shelter_id]["Identification"]["Cover"]:
                    dictree[p.shelter_id]["Identification"]["Cover"] = [
                        "{}/{}/{}".format(picpath, p.shelter_id, p.filename)
                    ]
                else:
                    dictree[p.shelter_id]["Identification"]["Cover"].append(
                        "{}/{}/{}".format(picpath, p.shelter_id, p.filename)
                    )
            elif not dictree[p.shelter_id][p.name]["Pictures"]:
                dictree[p.shelter_id][p.name]["Pictures"] = [
                    "{}/{}/{}".format(picpath, p.shelter_id, p.filename)
                ]
    ...
    **Example requests**:
    ...
    query = (
        db.session.query(
            ...
            func.array_agg(
                picpath + "/" + language_code + "/" + AttributePicture.file_name
            ).label("file_names"),
        )
...
def has_picture(uniqueid):
    ...
    **Example requests**:
    ...
    picpath = conf.ATTRIBUTES_PICTURES_SITE_PATH
    query = (
        db.session.query(AttributePicture.file_name)
        .join(Attribute)
        .filter(Attribute.uniqueid == uniqueid)
        .first()
    )
...
def getattributes(attribute_name, safetext=False):
    ...
    **Example requests**:
    ...
    attributes = (
        Attribute.query.filter(Attribute.uniqueid == attribute_name)
        .first()
        .associated_values
    )
    ...
    **Example requests**:
    ...
    querybase = (
        db.session.query(
            Property.shelter_id,
            Category.name.label("category_name"),
            Supercategory.name.label("supercategory_name"),
            Attribute.name,
            Attribute.uniqueid,
            Attribute.type,
            func.string_agg(Value.name, ";").label("value"),
        )
    ...
    picquerybase = db.session.query(
        ShelterPicture.shelter_id,
        ShelterPicture.file_name.label("filename"),
        ShelterPicture.is_main_picture,
        Category.name,
    ...
    docquerybase = db.session.query(
        ShelterDocument.shelter_id,
        ShelterDocument.file_name.label("filename"),
        ShelterDocument.category_id,
        Category.name,
    ...
    if shelter_id:
        ...
        shelter_documents = docquerybase.filter(
    ...
    if request.args.getlist("attribute"):
        ...
        subquery = (
            db.session.query(Property.shelter_id)
            .join(Attribute, Attribute.id == Property.attribute_id)
    ...
    if request.args.get("q"):
        ...
        shelter_properties = shelter_properties.join(
            Tsvector, Property.shelter_id == Tsvector.shelter_id
        ).filter(Tsvector.lexeme.match(attribute))
        shelter_pictures = shelter_pictures.join(
            Tsvector, ShelterPicture.shelter_id == Tsvector.shelter_id
        ).filter(Tsvector.lexeme.match(attribute))
        shelter_documents = shelter_documents.join(
            Tsvector, ShelterDocument.shelter_id == Tsvector.shelter_id
        ).filter(Tsvector.lexeme.match(attribute))
...
def latestshelters(count=1):
    ...
    **Example requests**:
    ...
    subsubquery = (
        db.session.query(ShelterPicture.shelter_id)
        .filter(ShelterPicture.is_main_picture == True)
        .subquery()
    )
    subquery = (
        db.session.query(Shelter)
    ...
    querybase = (
        db.session.query(
            subquery.c.id.label("shelter_id"),
            Category.name.label("category_name"),
            Supercategory.name.label("supercategory_name"),
            Attribute.name,
            Attribute.uniqueid,
